  ...
  const filterParsers = React.useMemo(() => {
    return getAllFilters.length
      ? getAllFilters.reduce<Record<string, Parser<string> | Parser<string[]>>>(
          (acc, field) => {
            const names = [
              ...(field.nameFrom ? [field.nameFrom] : []),
              ...(field.nameTo ? [field.nameTo] : []),
              ...(field.id ? [field.id] : []),
            ];
  
            names.forEach((name) => {
              if (field.type === "multi") {
                acc[name] = parseAsArrayOf(parseAsString, ",").withOptions(queryStateOptions);
              } else {
                acc[name] = parseAsString.withOptions(queryStateOptions);
              }
            });
