...
export const formComponentsMap = {
  ...
  datePickerRange: DatePickerWithRange,
};
...
console.log(defaultValues);
  ...
  const methods = useForm({
    ...
    mode: "onChange",
    defaultValues:{
      ...defaultValues, // existing form data
      image: typeof defaultValues?.image === 'string' ? defaultValues?.image.replace(/^"+|"+$/g, "") : "",
    },
  });
  console.log(
  ...
  const handleSubmit = async (body: any) => {
    ...
    if (sectionWithErrors) {
      ...
      toast.error("Please fix the errors before submitting", {
        description: `There are validation errors in the ${
          sections?.find((s) => s.id === sectionWithErrors)?.label
        } section`,
      });
      return;
    }
    SubmitForm({
      body,
      params: defaultValues ? { id: defaultValues?.id } : undefined,
    });
