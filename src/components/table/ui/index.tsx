...
export function StatusCellTrue({
  value,
  name,
  t,
  styled,
  ...
  const contextKey = `table.${name}`;
  const fallbackKey = `table.${isTrue ? "true" : "false"}`;

  let displayText = t(contextKey, {
    context: isTrue ? "true" : "false",
    defaultValue: "",
  });

  if (!displayText || displayText === contextKey) {
    displayText = t(fallbackKey);
  }
