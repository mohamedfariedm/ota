...
const TableFiltersSheet: React.FC<TableFiltersSheetProps> = ({
  ...
  const initialFiltersValues = sidebarFilters.reduce<Record<string, string>>(
    (acc, filter) => {
      if (filter.nameFrom) acc[filter.nameFrom] = "";
      if (filter.nameTo) acc[filter.nameTo] = "";
      if (filter.id && !filter.nameFrom && !filter.nameTo) acc[filter.id] = "";
      return acc;
    },
    {}
  );
            ...
            >
              ...
              <div className="flex justify-end py-6 mt-auto gap-5">
                <Button
                  ...
                  onClick={() => {
                    resetFilters();
                    methods.reset(initialFiltersValues); // أضف دي هنا
                  }}                >
                  Reset
                </Button>
                <SubmitBtnComponent
